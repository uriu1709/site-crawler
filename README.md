...
- GUI版（tkinter）をメインとし、PyInstallerで単体exeにビルド可能

## ファイル構成

```
サイトクローラー/
├── site_crawler_gui.py    # GUI版（メインアプリケーション、exe化対象）
├── site_crawler.py        # CLI版（初期の簡易スクリプト、メンテナンス対象外）
...
| 項目 | デフォルト値 | 説明 |
|------|------------|------|
| クロール開始URL | `https://www.bgu.ac.jp/` | クロールの起点URL |
| 出力CSVファイル | `~/Desktop/site_crawl_result.csv` | CSV出力先パス |
| 最大ページ数 | 2000 | クロール上限 |
...
`site_crawler.py` は初期開発時のCLI版スクリプト。
現在はGUI版（`site_crawler_gui.py`）がメインであり、CLI版はSSL対応・ログ出力・フィルタ機能等の改善が反映されていない。
CLI版はメンテナンス対象外。
